...
const SearchBar: React.FC = () => {
    ...
    const handleAddClick = (e: React.MouseEvent<HTMLDivElement>, track: TrackObject) => {
        ...
        const trackInPlaylist = addedTracks.includes(track.id);
    
        if (trackInPlaylist) {
            dispatch(removeTrackRequest({ playlistId: selectedPlaylistId, trackUri: track.uri }));
            setAddedTracks(addedTracks.filter(id => id !== track.id));
        } else {
                    ...
                    {!loading && !error && searchResults.length > 0 && (
                        <ul>
                            {searchResults.map((track) => {
                                const isAdded = addedTracks.includes(track.id); // Define isAdded
                                return (
                                    <li
                                        key={track.id}
